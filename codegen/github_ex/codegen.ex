defmodule GitHubEx.Codegen do
  ...
  defp build_operation(path, method, operation, components) do
    ...
    %{
      ...
      call: {Module.concat([GitHubEx, module_name]), function},
  ...
  defp render_module_source(module_name, operations) do
    ...
    """
    defmodule GitHubEx.#{module_name} do
      @moduledoc \"\"\"
  ...
  defp render_function_source(module_name, operation) do
    ...
    """
      @doc #{doc}
      @spec #{operation.function}(GitHubEx.Client.t()) :: result
      @spec #{operation.function}(GitHubEx.Client.t(), map()) :: result
      def #{operation.function}(client, params \\\\ %{}) when is_map(params) do
        GitHubEx.GeneratedSupport.execute(client, params, %{
          ...
          call: {GitHubEx.#{module_name}, :#{operation.function}},
