defmodule GitHubEx.GeneratedSourceTest do
  ...
  @support_source Path.expand("../../lib/github_ex/generated_support.ex", __DIR__)
  ...
  test "generated modules delegate through the shared support module" do
    ...
    assert Enum.all?(
             sources,
             &String.contains?(&1, "GitHubEx.GeneratedSupport.execute(client, params")
           )
  end
  ...
  test "request controls are handled in the shared support module" do
    ...
    assert support_source =~ "request_opts = GitHubEx.Client.request_opts(params)"
    assert support_source =~ "GitHubEx.Client.drop_request_controls(params)"
  ...
  test "runtime metadata is attached to generated request maps" do
    ...
    assert users_source =~ ~s(call: {GitHubEx.Users, :get_authenticated})
  end
end
