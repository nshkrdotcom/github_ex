defmodule GitHubSDK.GeneratedSupport do
  @moduledoc false

  @type key_spec :: {String.t(), atom()}
  @type payload_mode :: :none | :remaining | {:key, key_spec()}

  @spec execute(GitHubSDK.Client.t(), map(), map()) ::
          {:ok, term()} | {:error, GitHubSDK.Error.t()}
  def execute(%GitHubSDK.Client{} = client, params, spec)
      when is_map(params) and is_map(spec) do
    request_opts = GitHubSDK.Client.request_opts(params)

    auth =
      case spec[:auth_strategy] do
        :oauth_application -> GitHubSDK.Client.oauth_application_request_auth(params)
        _other -> Map.get(params, :auth) || Map.get(params, "auth")
      end

    params =
      case spec[:auth_strategy] do
        :oauth_application -> GitHubSDK.Client.drop_oauth_application_credentials(params)
        _other -> GitHubSDK.Client.drop_request_controls(params) |> drop_auth(params)
      end
    ...
    GitHubSDK.Client.execute_generated_request(client, %{
      args: params,
      auth: auth,
      body: body,
